...
{
	const auto AutoFormatting = FBAUtils::GetFormatterSettings(GetFocusedEdGraph()).GetAutoFormatting();
	...
	if (FBAFormatterSettings* FormatterSettings = FBAUtils::FindFormatterSettings(EdGraph))
	{
		switch (FormatterSettings->FormatterType)
		{
			...
			case EBAFormatterType::Simple:
				return MakeShared<FSimpleFormatter>(AsShared());
			default: ;
		}
	}
...
void FBAGraphHandler::SimpleFormatAll()
{
	...
	for (int i = 0; i < FormatAllColumns.Num(); ++i)
	{
		...
		for (UEdGraphNode* Node : FormatAllColumns[i])
		{
			...
			const int32 DeltaX = ColumnX - CurrentBounds.Left;
...
{
	// check if we should select the node as well
	if (NewPin)
	{
	...
	if (SelectedPinHandle.IsSet() && !(SelectedPinHandle.GetValue() == FBAGraphPinHandle(NewPin)))
	{
		TSharedPtr<SGraphPanel> GraphPanel = GetGraphPanel();
		if (GraphPanel.IsValid())
		{
			UEdGraphPin* EdGraphPin = SelectedPinHandle->GetPin();
			if (EdGraphPin != nullptr && !FBAUtils::IsNodeDeleted(EdGraphPin->GetOwningNodeUnchecked()))
			{
				TSharedPtr<SGraphPin> GraphPin = FBAUtils::GetGraphPin(GraphPanel, EdGraphPin);
				if (GraphPin.IsValid())
				{
