...
bool FBAUtils::IsExecPin(const UEdGraphPin* Pin)
{
	...
	if (FBAFormatterSettings* FormatterSettings = FindFormatterSettings(Graph))
	{
...
{
	UE_LOG(LogTemp, Warning, TEXT("%s"), *InitialMessage);
...
bool FBAUtils::IsValidGraph(UEdGraph* Graph)
{
	...
	if (FindFormatterSettings(Graph))
	{
		return true;
	}
...
TSharedPtr<SGraphNode> FBAUtils::GetHoveredGraphNode(TSharedPtr<SGraphPanel> GraphPanel)
{
	...
	for (UEdGraphNode* Node : Graph->Nodes)
	{
		...
		if (GraphNode.IsValid())
		{
			if (GraphNode->GetTickSpaceGeometry().IsUnderLocation(
...
{
	return (GraphPanel->GetCachedGeometry().LocalToAbsolute(GraphSpaceCoordinate) -
		GraphPanel->GetViewOffset()) * GraphPanel->GetZoomAmount();
}
...
void FBAUtils::InteractWithWidget(TSharedPtr<SWidget> Widget)
{
	...
	if (FBAUtils::IsClickableWidget(Widget))
	{
		...
		const bool bComboButton = Widget->GetTypeAsString() == "SComboButton" || Widget->GetTypeAsString() == "SComponentClassCombo";
		if (bComboButton)
		{
		...
		const bool bNeedsKeyboardEvent = Widget->GetTypeAsString() == "SCheckBox" || Widget->GetTypeAsString().Contains("Button");
...
bool FBAUtils::IsExtraRootNode(UEdGraphNode* Node)
{
	...
	if (FBAFormatterSettings* FormatterSettings = FindFormatterSettings(Node->GetGraph()))
	{
...
TArray<UEdGraphNode*> FBAUtils::GetNodesUnderCommentAndChildComments(UEdGraphNode_Comment* CommentNode)
{
	TSet<UEdGraphNode*> OutNodes;
	for (UObject* Obj : CommentNode->GetNodesUnderComment())
	{
		if (UEdGraphNode_Comment* ChildComment = Cast<UEdGraphNode_Comment>(Obj))
		{
			OutNodes.Add(ChildComment);
			OutNodes.Append(GetNodesUnderCommentAndChildComments(ChildComment));
		}
		else
		{
			if (UEdGraphNode* Node = Cast<UEdGraphNode>(Obj))
...
FSlateRect FBAUtils::GetCachedNodeArrayBoundsWithComments(TSharedPtr<FBAGraphHandler> GraphHandler, FCommentHandler* CommentHandler, const TArray<UEdGraphNode*>& Nodes)
{
	...
	for (UEdGraphNode_Comment* Comment : Comments)
	{
		...
		const FSlateRect CommentBounds = GetCommentBounds(CommentHandler, Comment);
...
FBAFormatterSettings* FBAUtils::FindFormatterSettings(UEdGraph* Graph)
{
	...
	if (IsBlueprintGraph(Graph, false))
	{
...
TSharedPtr<IMenu> FBAUtils::OpenPopupMenu(TSharedRef<SWidget> WindowContents, const FVector2D& PopupDesiredSize)
{
	...
	if (ParentWindow.IsValid())
	{
		...
		const FVector2D WindowPosition = MonitorCenter - PopupDesiredSize * 0.5f;
...
TSharedPtr<SGraphActionMenu> FBAUtils::GetGraphActionMenu()
{
	...
	TSharedPtr<SGraphActionMenu> ActionMenu = StaticCastSharedPtr<SGraphActionMenu>(ActionMenuWidget);
	return ActionMenu;
}

AActor* FBAUtils::GetSCSNodeDefaultActor(TSharedPtr<BA_SUBOBJECT_EDITOR_TREE_NODE> Node, UBlueprint* Blueprint)
{
#if BA_UE_VERSION_OR_LATER(5, 0)
...
bool FBAUtils::IsSCSActorNode(TSharedPtr<BA_SUBOBJECT_EDITOR_TREE_NODE> Node)
{
#if BA_UE_VERSION_OR_LATER(5, 0)
	...
	if (UBlueprint* Blueprint = Cast<UBlueprint>(Graph->GetOuter()))
	{
		if (UAssetEditorSubsystem* AssetEditorSubsystem = GEditor->GetEditorSubsystem<UAssetEditorSubsystem>())
		{
			if (IAssetEditorInstance* AssetEditor = AssetEditorSubsystem->FindEditorForAsset(Blueprint, false))
			{
				...
				{
					return static_cast<FBlueprintEditor*>(AssetEditor);
				}
			}
		}
	}
