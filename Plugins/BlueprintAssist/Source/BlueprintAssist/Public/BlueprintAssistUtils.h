...
#include "BlueprintAssistSettings.h" // needed for FBAFormatterSettings
...
struct BLUEPRINTASSIST_API FBAUtils
{
	...
	static TArray<UEdGraphNode*> GetNodesUnderCommentAndChildComments(UEdGraphNode_Comment* CommentNode);
	...
	static FSlateRect GetCachedNodeArrayBoundsWithComments(TSharedPtr<FBAGraphHandler> GraphHandler, FCommentHandler* CommentHandler, const TArray<UEdGraphNode*>& Nodes);
	...
	static EditorClass* GetEditorFromActiveTabCasted()
	{
		...
		{
			return RootObject->GetAssetHandler()->GetEditorFromTabCasted<AssetClass, EditorClass>(FBATabHandler::Get().GetLastMajorTab());
		}
	...
	static AssetClass* GetAssetForActiveTab()
	{
		if (UBARootObject* RootObject = FBlueprintAssistModule::Get().GetRootObject())
		{
			return RootObject->GetAssetHandler()->GetAssetFromTab<AssetClass>(FBATabHandler::Get().GetLastMajorTab());
		}
